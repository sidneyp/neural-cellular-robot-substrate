...
class ModularRobot:
  def __init__(self, world, body_grid, init_x=0, init_y=0, init_angle=0):
    ...
    hull_mask = np.logical_or(body_grid == 1, body_grid == 2)
    wheel_vertical_mask = body_grid == 3
    wheel_horizontal_mask = body_grid == 4
    wheel_mask = np.logical_or(wheel_vertical_mask, wheel_horizontal_mask)
    sensor_mask = body_grid == 2
    assert (np.sum(sensor_mask) > 0 and np.sum(wheel_mask) >=2)
    ...
    for x, y in hull_module_pos:
      hull_fixtures.append(fixtureDef(
        shape=polygonShape(
          ...
          # vertices=[(x+mx, y+my) for mx, my in MODULE_POLY]
          ),
        density=1.0,
        )
      )
    ...
    # Sensors
    sensor_pos = module_pos[sensor_mask]
    self.sensors = []
    ...
    self.drawlist = self.wheels + [self.hull] + self.sensors
